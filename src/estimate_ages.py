...
def estimate_age_from_categorical(df): 
    ...
    ages_estimated, age_groups, age_group_cts = generate_age_estimates(subject_age_group)
...
def generate_age_estimates(subject_age_group):
    ...
    avg, std, age_groups, age_group_cts = calculate_mean_and_std(subject_age_group)    
...
def calculate_mean_and_std(subject_age_group):
    ...
    as common. These values are seen as red dots in the histogram
    output by this file.  For example, for 100 individuals in the age range
    from 25-35, there are exactly 10 of age 25, 10 of age 26, 10 of age 27, etc.
    ...
    for age_group in AGE_STRING_TO_LIST.keys():
        ...
        for item in age_range_list:
            ...
            ages_and_their_counts[item] = int(age_group_counts[age_group]/len(age_range_list))
